-- ============================================================================
-- P1.3.4-B-S2-B-PHASE1-CORRECTION — Correction sécurité role_permissions
-- ============================================================================
-- Corrige les 3 RPC : grant_role_permission, revoke_role_permission, get_role_permission
-- Remplace MANAGE par GRANT, ajoute anti-auto-élévation, sécurise get_role_permission
-- NE MODIFIE PAS les données, permissions existantes, ni RLS.

-- Extension UUID (déjà présente)
create extension if not exists "pgcrypto";

-- ============================================================================
-- 1. CORRECTION grant_role_permission
-- ============================================================================
-- Remplace la fonction existante par la version corrigée
...
begin
    ...
    -- (MANAGE ne suffit plus — GRANT = attribuer des capabilities à d'autres rôles)
    ...
    -- (directement OU via délégation reçue)
    -- Si oui, interdire d'ajouter GRANT ou DELEGATE sur les permissions RBAC sensibles
...
-- ============================================================================
-- 2. CORRECTION revoke_role_permission
-- ============================================================================
...
begin
    ...
    -- (directement OU via délégation reçue) — utilise l'autorité effective S1
    v_actor_has_target_role := public.has_effective_capability(
        v_actor_uid,
        'USE',
        'role',
...
si l''acteur a le rôle cible effectivement (direct ou délégation). S''applique à tous, Super Admin inclus.';
...
-- ============================================================================
-- 3. CORRECTION get_role_permission
-- ============================================================================
-- Ajoute contrôle d'autorisation (USE/MANAGE/GRANT/DELEGATE sur rbac.role_permissions)
-- et paramètre de scope obligatoire

drop function if exists public.get_role_permission(text, text);
...
begin
    ...
    -- 3. Vérifier autorité effective : USE OU MANAGE OU GRANT OU DELEGATE sur rbac.role_permissions
    -- Une seule suffit pour la lecture
    ...
    if not v_actor_has_authority then
        raise exception 'PERMISSION_INSUFFISANTE: USE/MANAGE/GRANT/DELEGATE sur rbac.role_permissions requis dans le scope %:%', p_scope_type, coalesce(p_scope_value, 'global');
    end if;
    ...
    -- 6. Lookup unique (pas d'énumération)
    ...
    if v_result is null then
        -- Pas de ligne = pas de capabilities (équivaut à toutes false)
...
comment on function public.get_role_permission(text, text, text, text) is
'Retourne les 4 capacités d''un rôle pour une permission donnée.
Nécessite : authentification + USE/MANAGE/GRANT/DELEGATE effectif sur rbac.role_permissions dans le scope.
Lookup unique (role_id, permission_id) — pas d''énumération.';

grant execute on function public.get_role_permission(text, text, text, text) to authenticated;
revoke execute on function public.get_role_permission(text, text, text, text) from anon;

-- ============================================================================
-- FIN CORRECTION PHASE 1
-- ============================================================================